...
exports.BitstreamElement = void 0;
...
class BitstreamElement {
    ...
    serialize(fromRef, toRef, autoPad = false) {
        ...
        let stream = new common_1.BufferedWritable();
        ...
        let writer = (_a = this.constructor[SERIALIZE_WRITERS].pop()) !== null && _a !== void 0 ? _a : new bitstream_1.BitstreamWriter(stream, this.serializeBufferSize);
    ...
    measure(fromRef, toRef) {
        ...
        let fromIndex = this.syntax.findIndex(x => x === from);
        let toIndex = this.syntax.findIndex(x => x === to);
        let measurer = new bitstream_1.BitstreamMeasurer();
        if (fromIndex > toIndex) {
            throw new Error(`Cannot measure from field ${fromIndex} (${String(from.name)}) to ${toIndex} (${String(to.name)}): First field comes after last field`);
        }
        for (let i = fromIndex, max = toIndex; i <= max; ++i) {
    ...
    *readGroup(bitstream, name, options) {
    ...
    writeGroup(bitstream, name, options) {
        ...
        for (let element of syntax) {
            ...
            if (globalThis.BITSTREAM_TRACE === true && !(bitstream instanceof bitstream_1.BitstreamMeasurer)) {
    ...
    *variate(reader, parent, field) {
    ...
    static deserialize(data, options = {}) {
        let reader = new bitstream_1.BitstreamReader();
...
exports.BitstreamElement = BitstreamElement;
