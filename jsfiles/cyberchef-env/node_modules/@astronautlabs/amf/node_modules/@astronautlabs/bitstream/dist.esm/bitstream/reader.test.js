...
describe('BitstreamReader', it => {
    ...
    it('.readSignedSync() correctly handles signed integers', () => {
        let bitstream = new BitstreamReader();
        bitstream.addBuffer(Buffer.from([0xFB]));
        expect(bitstream.readSignedSync(8)).to.equal(-5);
        bitstream.addBuffer(Buffer.from([5]));
        expect(bitstream.readSignedSync(8)).to.equal(5);
        bitstream.addBuffer(Buffer.from([0]));
        expect(bitstream.readSignedSync(8)).to.equal(0);
        bitstream.addBuffer(Buffer.from([0xFC, 0x0A]));
        expect(bitstream.readSignedSync(16)).to.equal(-1014);
        bitstream.addBuffer(Buffer.from([0x03, 0xF6]));
        expect(bitstream.readSignedSync(16)).to.equal(1014);
        bitstream.addBuffer(Buffer.from([0, 0]));
        expect(bitstream.readSignedSync(16)).to.equal(0);
        bitstream.addBuffer(Buffer.from([0xFF, 0xFE, 0x70, 0x40]));
        expect(bitstream.readSignedSync(32)).to.equal(-102336);
        bitstream.addBuffer(Buffer.from([0x00, 0x01, 0x8F, 0xC0]));
        expect(bitstream.readSignedSync(32)).to.equal(102336);
        bitstream.addBuffer(Buffer.from([0, 0, 0, 0]));
        expect(bitstream.readSignedSync(32)).to.equal(0);
    });
    it('.readSigned() correctly handles signed integers', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        bitstream.addBuffer(Buffer.from([0xFB]));
        expect(yield bitstream.readSigned(8)).to.equal(-5);
        bitstream.addBuffer(Buffer.from([5]));
        expect(yield bitstream.readSigned(8)).to.equal(5);
        bitstream.addBuffer(Buffer.from([0]));
        expect(yield bitstream.readSigned(8)).to.equal(0);
        bitstream.addBuffer(Buffer.from([0xFC, 0x0A]));
        expect(yield bitstream.readSigned(16)).to.equal(-1014);
        bitstream.addBuffer(Buffer.from([0x03, 0xF6]));
        expect(yield bitstream.readSigned(16)).to.equal(1014);
        bitstream.addBuffer(Buffer.from([0, 0]));
        expect(yield bitstream.readSigned(16)).to.equal(0);
        bitstream.addBuffer(Buffer.from([0xFF, 0xFE, 0x70, 0x40]));
        expect(yield bitstream.readSigned(32)).to.equal(-102336);
        bitstream.addBuffer(Buffer.from([0x00, 0x01, 0x8F, 0xC0]));
        expect(yield bitstream.readSigned(32)).to.equal(102336);
        bitstream.addBuffer(Buffer.from([0, 0, 0, 0]));
        expect(yield bitstream.readSigned(32)).to.equal(0);
    }));
    it('.readSigned() can wait until data is available', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
    ...
    it('.readFloatSync() correctly handles floats', () => {
        let bitstream = new BitstreamReader();
        bitstream.addBuffer(Buffer.from([0x42, 0xCD, 0x00, 0x00]));
        expect(bitstream.readFloatSync(32)).to.equal(102.5);
        bitstream.addBuffer(Buffer.from([0xC3, 0xDA, 0x00, 0x00]));
        expect(bitstream.readFloatSync(32)).to.equal(-436);
        bitstream.addBuffer(Buffer.from([0, 0, 0, 0]));
        expect(bitstream.readFloatSync(32)).to.equal(0);
    ...
    it('.readFloat() correctly handles floats', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        bitstream.addBuffer(Buffer.from([0x42, 0xCD, 0x00, 0x00]));
        expect(yield bitstream.readFloat(32)).to.equal(102.5);
        bitstream.addBuffer(Buffer.from([0xC3, 0xDA, 0x00, 0x00]));
        expect(yield bitstream.readFloat(32)).to.equal(-436);
        bitstream.addBuffer(Buffer.from([0, 0, 0, 0]));
        expect(yield bitstream.readFloat(32)).to.equal(0);
        bitstream.addBuffer(Buffer.from([0x41, 0x60, 0xae, 0x29, 0x71, 0xeb, 0x85, 0x1f]));
        expect(yield bitstream.readFloat(64)).to.equal(8745291.56);
        bitstream.addBuffer(Buffer.from([0xc1, 0x14, 0x00, 0xa4, 0xae, 0x14, 0x7a, 0xe1]));
        expect(yield bitstream.readFloat(64)).to.equal(-327721.17);
        bitstream.addBuffer(Buffer.from([0, 0, 0, 0, 0, 0, 0, 0]));
        expect(yield bitstream.readFloat(64)).to.equal(0);
    }));
    it('.readFloat() can wait until data is available', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
    ...
    it('peek() reads an unsigned integer without consuming it', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        bitstream.addBuffer(Buffer.from([37]));
        expect(yield bitstream.peek(8)).to.equal(37);
        expect(bitstream.offset).to.equal(0);
        expect(yield bitstream.peek(8)).to.equal(37);
        expect(yield bitstream.peek(8)).to.equal(37);
        expect(yield bitstream.peek(8)).to.equal(37);
    ...
    it('.read() fast paths when enough bits are available', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        bitstream.addBuffer(Buffer.alloc(1));
        let promise = bitstream.read(8);
        expect(bitstream['blockedRequest']).to.be.null;
        expect(yield promise).to.equal(0);
    }));
    it('.read() can wait until enough bits are available', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        setTimeout(() => bitstream.addBuffer(Buffer.from([12])), 10);
        expect(yield bitstream.read(8)).to.equal(12);
    }));
    it('correctly handles NaN', () => {
        let bitstream = new BitstreamReader();
        bitstream.addBuffer(Buffer.from([0x7F, 0xC0, 0x00, 0x00]));
        expect(bitstream.readFloatSync(32)).to.be.NaN;
    ...
    it('.read() allows only one async read at a time', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        bitstream.read(8);
        let caught;
        try {
            yield bitstream.read(8);
        }
        catch (e) {
            caught = e;
        }
    ...
    it('.assure() allows only one async call at a time', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        bitstream.assure(8);
        let caught;
        try {
            yield bitstream.assure(8);
        }
        catch (e) {
            caught = e;
        }
    ...
    it('.readString() reads a string correctly when the data is already available', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        let buf = Buffer.alloc(10);
        buf.write('hello', 'utf-8');
        bitstream.addBuffer(buf);
        expect(yield bitstream.readString(10)).to.equal('hello');
    }));
    it('.readString() reads a string correctly when the data is not yet available', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        let buf = Buffer.alloc(10);
        buf.write('hello', 'utf-8');
    ...
    it('.readBytes() reads a buffer correctly when the data is already available', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        let buf = Buffer.from([12, 42, 15]);
        bitstream.addBuffer(buf);
        let buf2 = Buffer.alloc(3);
        yield bitstream.readBytesBlocking(buf2);
    ...
    it('.addBuffer() throws when called on an ended stream', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        expect(() => bitstream.addBuffer(new Uint8Array(1))).not.to.throw;
        bitstream.end();
        expect(() => bitstream.addBuffer(new Uint8Array(1))).to.throw;
    }));
    it('.simulate() restores offset after execution', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        bitstream.addBuffer(new Uint8Array(2));
        bitstream.readSync(4);
        yield bitstream.simulate(() => __awaiter(void 0, void 0, void 0, function* () {
            bitstream.readSync(4);
            expect(bitstream.offset).to.equal(8);
        }));
        expect(bitstream.offset).to.equal(4);
    }));
    it('.simulateSync() can be nested', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        ...
        yield bitstream.simulate(() => __awaiter(void 0, void 0, void 0, function* () {
            bitstream.readSync(4);
            expect(bitstream.offset).to.equal(8);
            yield bitstream.simulateSync(() => __awaiter(void 0, void 0, void 0, function* () {
                bitstream.readSync(4);
    ...
    it('.simulateSync() restores offset after execution', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        bitstream.addBuffer(new Uint8Array(2));
        bitstream.readSync(4);
    ...
    it('.end() markes stream as ended', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        expect(bitstream.ended).to.be.false;
        bitstream.end();
        expect(bitstream.ended).to.be.true;
    }));
    it('.end() causes pending assure() to reject', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        let thrown = bitstream.assure(8).then(() => false).catch(() => true);
        bitstream.end();
        expect(yield thrown).to.be.true;
    }));
    it('.end() causes pending read() to reject', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        let thrown = bitstream.read(8).then(() => false).catch(() => true);
        bitstream.end();
        expect(yield thrown).to.be.true;
    }));
    it('.end() does not cause a pending optional assure() to reject', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        let thrown = bitstream.assure(8, true).then(() => false).catch(() => true);
        bitstream.end();
        expect(yield thrown).to.be.false;
    }));
    it('.reset() throws if there is a pending operation', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        let thrown = bitstream.assure(8, true).then(() => false).catch(() => true);
        expect(() => bitstream.reset()).to.throw;
    }));
    it('.reset() does not throw if there is no pending operation', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        expect(() => bitstream.reset()).not.to.throw;
    }));
    it('.reset() unmarks a stream as ended', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
        expect(() => bitstream.addBuffer(new Uint8Array(1))).not.to.throw;
        bitstream.end();
        expect(bitstream.ended).to.be.true;
    ...
    it('.reset() clears buffers and resets read head to zero', () => __awaiter(void 0, void 0, void 0, function* () {
        let bitstream = new BitstreamReader();
...
describe('BitstreamReader (generated)', it => {
    for (let size = 1; size <= 52; ++size) {
        it(`reads ${size}bit values correctly`, () => __awaiter(void 0, void 0, void 0, function* () {
            let offset = 64 - size;
    ...
    for (let size = 1; size < 52; ++size) {
        it(`reads cross-byte ${size}bit values correctly [1-bit offset]`, () => __awaiter(void 0, void 0, void 0, function* () {
            let offset = 64 - size;
            let buf = new ArrayBuffer(8);
            let view = new DataView(buf);
            for (let i = 0; i < 10; ++i) {
    ...
    for (let size = 1; size < 49; ++size) {
        it(`reads cross-byte ${size}bit values correctly [4-bit offset]`, () => __awaiter(void 0, void 0, void 0, function* () {
            let offset = 64 - size;
            let buf = new ArrayBuffer(8);
            let view = new DataView(buf);
            for (let i = 0; i < 10; ++i) {
