...
describe('BitstreamWriter', it => {
    it('works for bit writes', () => {
        let bufs = [];
        let fakeStream = { write(buf) { bufs.push(buf); } };
    ...
    it('correctly handles signed integers', () => {
        ...
        let fakeStream = { write(buf) { bufs.push(buf); } };
        let writer = new BitstreamWriter(fakeStream);
        writer.writeSigned(8, -5);
        expect(bufs[0][0]).to.equal(0xFB);
        writer.writeSigned(8, 5);
        expect(bufs[1][0]).to.equal(5);
        writer.writeSigned(8, 0);
        expect(bufs[2][0]).to.equal(0);
        bufs = [];
        writer = new BitstreamWriter(fakeStream, 2);
        writer.writeSigned(16, -1014);
        expect(Array.from(bufs[0])).to.eql([0xFC, 0x0A]);
        writer.writeSigned(16, 1014);
        expect(Array.from(bufs[1])).to.eql([0x03, 0xF6]);
        writer.writeSigned(16, 0);
        expect(Array.from(bufs[2])).to.eql([0, 0]);
        bufs = [];
        writer = new BitstreamWriter(fakeStream, 4);
        writer.writeSigned(32, -102336);
        expect(Array.from(bufs[0])).to.eql([0xFF, 0xFE, 0x70, 0x40]);
        writer.writeSigned(32, 102336);
        expect(Array.from(bufs[1])).to.eql([0x00, 0x01, 0x8F, 0xC0]);
        writer.writeSigned(32, 0);
        expect(Array.from(bufs[2])).to.eql([0, 0, 0, 0]);
    });
    it('correctly handles floats', () => {
        let bufs = [];
        let fakeStream = { write(buf) { bufs.push(buf); } };
        let writer = new BitstreamWriter(fakeStream, 4);
        writer.writeFloat(32, 102.5);
        expect(Array.from(bufs[0])).to.eql([0x42, 0xCD, 0x00, 0x00]);
        writer.writeFloat(32, -436);
        expect(Array.from(bufs[1])).to.eql([0xC3, 0xDA, 0x00, 0x00]);
    ...
    it('correctly handles NaN', () => {
        ...
        let fakeStream = { write(buf) { bufs.push(buf); } };
        let writer = new BitstreamWriter(fakeStream, 4);
        writer.writeFloat(32, NaN);
        expect(Array.from(bufs[0])).to.eql([0x7F, 0xC0, 0x00, 0x00]);
        bufs = [];
        writer = new BitstreamWriter(fakeStream, 8);
    ...
    it('correctly handles Infinity', () => {
        ...
        let fakeStream = { write(buf) { bufs.push(buf); } };
        let writer = new BitstreamWriter(fakeStream, 4);
        writer.writeFloat(32, Infinity);
        expect(Array.from(bufs[0])).to.eql([0x7f, 0x80, 0x00, 0x00]);
        bufs = [];
        writer = new BitstreamWriter(fakeStream, 8);
