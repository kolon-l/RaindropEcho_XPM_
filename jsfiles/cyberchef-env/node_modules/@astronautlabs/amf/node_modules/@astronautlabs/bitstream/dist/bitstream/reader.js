...
class BitstreamReader {
    ...
    set offset(value) {
        if (value < this._spentBufferSize) {
            throw new Error(`Offset ${value} points into a discarded buffer! `
     ...
     */
    readString(length, options) {
        return __awaiter(this, void 0, void 0, function* () {
            this.ensureNoReadPending();
            yield this.assure(8 * length);
    ...
    readStringSync(length, options) {
        ...
        let charLength = 1;
        let encoding = (_a = options.encoding) !== null && _a !== void 0 ? _a : 'utf-8';
        ...
        for (let i = 0, max = length; i < max; i += charLength) {
            let char = buffer[i];
            if (charLength === 2)
                char = (char << 8) | ((_b = buffer[i + 1]) !== null && _b !== void 0 ? _b : 0);
            if (char === 0) {
                firstTerminator = i;
                break;
            }
     ...
     */
    *readBytes(buffer, offset = 0, length) {
        var _a;
        length !== null && length !== void 0 ? length : (length = buffer.length - offset);
        ...
        if (bitOffset === 0) {
            if (globalThis.BITSTREAM_TRACE) {
                ...
                console.log(`------------------------------------------------------------    readBytes(): Pre-operation: buffered=${this.bufferedLength} bits, bufferIndex=${this._bufferIndex}, bufferOffset=${this._offsetIntoBuffer}, bufferLength=${((_a = this.buffers[this._bufferIndex]) === null || _a === void 0 ? void 0 : _a.length) || '<none>'} bufferCount=${this.buffers.length}`);
            }
     ...
     * @param length The length of bytes to read. Defaults to the length of the array (sans the offset)
     */
    readBytesSync(buffer, offset = 0, length) {
        length !== null && length !== void 0 ? length : (length = buffer.length - offset);
     ...
     * @param length The length of bytes to read. Defaults to the length of the array (sans the offset)
     */
    readBytesBlocking(buffer, offset = 0, length) {
        return __awaiter(this, void 0, void 0, function* () {
            length !== null && length !== void 0 ? length : (length = buffer.length - offset);
            let gen = this.readBytes(buffer, offset, length);
            while (true) {
                let result = gen.next();
                if (result.done === false)
                    yield this.assure(result.value * 8);
                else
                    break;
            }
            return buffer;
    ...
    maskOf(bits) {
        ...
        return (_a = maskMap.get(bits)) !== null && _a !== void 0 ? _a : (Math.pow(0x2, bits) - 1);
    }
    /**
    ...
    readShortByteAligned(consume, byteOrder) {
        let buffer = this.buffers[this._bufferIndex];
    ...
    readLongByteAligned(consume, byteOrder) {
    ...
    read3ByteAligned(consume, byteOrder) {
        let bufferIndex = this._bufferIndex;
        let buffer = this.buffers[bufferIndex];
        let bufferOffset = this._offsetIntoBuffer / 8;
        let firstByte = buffer[bufferOffset++];
    ...
    readPartialByte(length, consume) {
        let buffer = this.buffers[this._bufferIndex];
     ...
     * @returns
     */
    readCoreSync(length, consume, byteOrder = 'msb') {
        this.ensureNoReadPending();
    ...
    block(request) {
        ...
        this.blockedRequest = Object.assign(Object.assign({}, request), this.promise());
